...
namespace
{
    ...
    bool EncodeSingleFrame(IWICImagingFactory* pFactory, REFGUID containerFormat,
                           const std::string& filePath, ConfigureFrameFn configureFrame)
    {
        ...
        if (FAILED(hr)) { pEncoder->Release(); pStream->Release(); return false; }
        ...
        if (ok) ok = SUCCEEDED(pFrame->Commit());
        if (ok) ok = SUCCEEDED(pEncoder->Commit());

        if (pFrameProps) pFrameProps->Release();
        pFrame->Release();
        pEncoder->Release();
        pStream->Release();
        return ok;
...
bool SavePNG(const std::string& filePath, const uint8_t* pixels, int width, int height, int stride)
{
    ...
    return EncodeSingleFrame(pFactory, GUID_ContainerFormatPng, filePath,
        ...
        {
            if (pProps) pFrame->Initialize(pProps); else pFrame->Initialize(nullptr);
            pFrame->SetSize(width, height);

            WICPixelFormatGUID fmt = GUID_WICPixelFormat24bppBGR;
            if (FAILED(pFrame->SetPixelFormat(&fmt))) return false;
...
bool SaveIndexedPNG(const std::string& filePath, const uint8_t* pixels, int width, int height, int stride)
{
    ...
    if (FAILED(pFactory->CreatePalette(&pWICPalette))) return false;
    HRESULT hr = pWICPalette->InitializeCustom(palette.data(), static_cast<UINT>(palette.size()));
    if (FAILED(hr)) { pWICPalette->Release(); return false; }
    ...
    bool ok = EncodeSingleFrame(pFactory, GUID_ContainerFormatPng, filePath,
        [&](IWICImagingFactory*, IWICBitmapEncoder* pEncoder, IWICBitmapFrameEncode* pFrame, IPropertyBag2* pProps) -> bool
        {
            ...
            if (FAILED(pFrame->SetPalette(pWICPalette))) return false;

            UINT indexedStride = width;
            return SUCCEEDED(pFrame->WritePixels(height, indexedStride, indexedStride * height,
                                                 const_cast<BYTE*>(indexed.data())));
        });
