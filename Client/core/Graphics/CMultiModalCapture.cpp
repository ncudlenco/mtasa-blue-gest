...
static void SegDiagLog(bool enabled, const char* fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    SegDiagLogV(enabled, fmt, ap);
    va_end(ap);
}

// Forward declaration — definition lives further down near RenderDepthMap.
static bool TryInstallIntz(IDirect3DDevice9* pDevice, int width, int height, IDirect3DTexture9*& outTexture, IDirect3DSurface9*& outSurface,
...
bool CMultiModalCapture::Initialize(IDirect3DDevice9* pDevice, int width, int height)
{
    ...
    HRESULT           hr =
        D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, 0, nullptr, 0, D3D11_SDK_VERSION, &m_pD3D11Device, &featureLevel, &m_pD3D11Context);
    if (FAILED(hr))
    {
    ...
    m_bIntzInstalled = TryInstallIntz(pDevice, width, height, m_pIntzTexture, m_pIntzSurface, m_pOriginalDepthStencil);
    ...
    m_bInitialized = true;
    SegDiagLog(m_bDiagLogsEnabled, "[Seg/Init] DONE RGB=%p Seg=%p SegSnap=%p SegDS=%p Depth=%p intz=%d pDevice=%p\n", static_cast<void*>(m_pRGBSurface),
               static_cast<void*>(m_pSegmentationSurface), static_cast<void*>(m_pSegmentationSnapshot), static_cast<void*>(m_pSegDepthStencil),
               static_cast<void*>(m_pDepthSurface), m_bIntzInstalled ? 1 : 0, static_cast<void*>(m_pDevice));
    return true;
}
...
void CMultiModalCapture::Shutdown()
{
    ...
    if (m_pIntzSurface)
    {
        m_pIntzSurface->Release();
        m_pIntzSurface = nullptr;
    }
    if (m_pIntzTexture)
    {
        m_pIntzTexture->Release();
        m_pIntzTexture = nullptr;
    }
...
bool CMultiModalCapture::CreateRenderTargets(int width, int height)
{
    ...
    hr = m_pDevice->CreateRenderTarget(width, height, D3DFMT_X8R8G8B8, D3DMULTISAMPLE_NONE, 0, FALSE, &m_pRGBSurface, nullptr);
    SegDiagLog(m_bDiagLogsEnabled, "[Seg/CreateRT] RGB    hr=0x%08X surf=%p\n", static_cast<unsigned>(hr), static_cast<void*>(m_pRGBSurface));
    if (FAILED(hr))
    {
        OutputDebugString("[CMultiModalCapture] RGB RT create failed\n");
        return false;
    }

    hr = m_pDevice->CreateRenderTarget(width, height, D3DFMT_X8R8G8B8, D3DMULTISAMPLE_NONE, 0, FALSE, &m_pSegmentationSurface, nullptr);
    ...
    if (FAILED(hr))
    {
        OutputDebugString("[CMultiModalCapture] Seg RT create failed\n");
        ReleaseRenderTargets();
        return false;
    }

    hr = m_pDevice->CreateRenderTarget(width, height, D3DFMT_X8R8G8B8, D3DMULTISAMPLE_NONE, 0, FALSE, &m_pSegmentationSnapshot, nullptr);
    ...
    if (FAILED(hr))
    {
        OutputDebugString("[CMultiModalCapture] Seg snapshot RT create failed\n");
        ReleaseRenderTargets();
        return false;
    }

    hr = m_pDevice->CreateRenderTarget(width, height, D3DFMT_X8R8G8B8, D3DMULTISAMPLE_NONE, 0, FALSE, &m_pDepthSurface, nullptr);
    ...
    if (FAILED(hr))
    {
        OutputDebugString("[CMultiModalCapture] Depth RT create failed\n");
        ReleaseRenderTargets();
        return false;
    }
    ...
    hr = m_pDevice->CreateDepthStencilSurface(width, height, D3DFMT_D24S8, D3DMULTISAMPLE_NONE, 0, TRUE, &m_pSegDepthStencil, nullptr);
    ...
    if (FAILED(hr))
    {
        OutputDebugString("[CMultiModalCapture] Seg DS create failed\n");
        ReleaseRenderTargets();
        return false;
    }
...
void CMultiModalCapture::ReleaseRenderTargets()
{
    if (m_pRGBSurface)
    {
        m_pRGBSurface->Release();
        m_pRGBSurface = nullptr;
    }
    if (m_pSegmentationSurface)
    {
        m_pSegmentationSurface->Release();
        m_pSegmentationSurface = nullptr;
    }
    if (m_pSegmentationSnapshot)
    {
        m_pSegmentationSnapshot->Release();
        m_pSegmentationSnapshot = nullptr;
    }
    if (m_pSegDepthStencil)
    {
        m_pSegDepthStencil->Release();
        m_pSegDepthStencil = nullptr;
    }
    if (m_pDepthSurface)
    {
...
static bool CompilePixelShader(IDirect3DDevice9* pDevice, const char* source, size_t sourceLen, IDirect3DPixelShader9** outShader, const char* label)
{
    ID3DXBuffer* pCode = nullptr;
    ID3DXBuffer* pErr = nullptr;
    HRESULT      hr = D3DXCompileShader(source, sourceLen, nullptr, nullptr, "main", "ps_2_0", 0, &pCode, &pErr, nullptr);
    if (FAILED(hr))
    {
        if (pErr)
        {
...
static bool CompileVertexShader(IDirect3DDevice9* pDevice, const char* source, size_t sourceLen, IDirect3DVertexShader9** outShader, const char* label)
{
    ID3DXBuffer* pCode = nullptr;
    ID3DXBuffer* pErr = nullptr;
    HRESULT      hr = D3DXCompileShader(source, sourceLen, nullptr, nullptr, "main", "vs_2_0", 0, &pCode, &pErr, nullptr);
    if (FAILED(hr))
    {
        if (pErr)
        {
...
bool CMultiModalCapture::CreateShaders(IDirect3DDevice9* pDevice)
{
    ...
    if (!CompileVertexShader(pDevice, kFullscreenQuadVS, sizeof(kFullscreenQuadVS) - 1, &m_pFullscreenQuadVS, "Fullscreen quad VS"))
        return false;

    if (!CompilePixelShader(pDevice, kDepthVisPS, sizeof(kDepthVisPS) - 1, &m_pDepthVisualizationShader, "Depth PS"))
        return false;

    if (!CompilePixelShader(pDevice, kConstantColorPS, sizeof(kConstantColorPS) - 1, &m_pConstantColorShader, "Segmentation PS"))
        return false;
...
{
    if (m_pDepthVisualizationShader)
    {
        m_pDepthVisualizationShader->Release();
        m_pDepthVisualizationShader = nullptr;
    }
    if (m_pFullscreenQuadVS)
    {
        m_pFullscreenQuadVS->Release();
        m_pFullscreenQuadVS = nullptr;
    }
    if (m_pConstantColorShader)
    {
        m_pConstantColorShader->Release();
        m_pConstantColorShader = nullptr;
    }
}
...
bool CMultiModalCapture::CreateFullscreenQuad(IDirect3DDevice9* pDevice)
{
    ...
    HRESULT hr = pDevice->CreateVertexBuffer(sizeof(verts), D3DUSAGE_WRITEONLY, kQuadFVF, D3DPOOL_MANAGED, &m_pFullscreenQuadVB, nullptr);
    if (FAILED(hr))
        return false;
...
{
    if (!pDevice || !m_pSegmentationSurface)
        return false;
                           ...
                           IDirect3DSurface9*& outOriginalDS)
{
    ...
    if (FAILED(pDevice->GetDirect3D(&pD3D)) || !pD3D)
        return false;
    ...
    D3DDISPLAYMODE mode;
    if (FAILED(pD3D->GetAdapterDisplayMode(params.AdapterOrdinal, &mode)))
    {
        pD3D->Release();
        return false;
    }

    HRESULT hr = pD3D->CheckDeviceFormat(params.AdapterOrdinal, params.DeviceType, mode.Format, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_TEXTURE, INTZ_FORMAT);
    pD3D->Release();
    if (FAILED(hr))
    {
    ...
    hr = pDevice->CreateTexture(width, height, 1, D3DUSAGE_DEPTHSTENCIL, INTZ_FORMAT, D3DPOOL_DEFAULT, &pTex, nullptr);
    if (FAILED(hr) || !pTex)
        return false;
    ...
    if (FAILED(hr))
    {
        if (outOriginalDS)
        {
            outOriginalDS->Release();
            outOriginalDS = nullptr;
        }
        pSurf->Release();
        pTex->Release();
        return false;
...
CVideoEncoder* CMultiModalCapture::EncoderForModality(int modalityId)
{
    ...
    {
        case EModality::RGB:
            return &m_RGBVideoEncoder;
        case EModality::SEGMENTATION:
            return &m_SegmentationVideoEncoder;
        case EModality::DEPTH:
            return &m_DepthVideoEncoder;
    }
    return nullptr;
}

bool CMultiModalCapture::StartVideoRecording(int modalityId, const std::string& videoPath, int width, int height, int fps, int bitrate)
{
    if (!m_bInitialized)
        return false;
...
{
    if (!pRTSurface || !m_pDevice)
        return false;

    D3DSURFACE_DESC desc;
    if (FAILED(pRTSurface->GetDesc(&desc)))
        return false;

    IDirect3DSurface9* pSysMem = nullptr;
    HRESULT            hr = m_pDevice->CreateOffscreenPlainSurface(desc.Width, desc.Height, desc.Format, D3DPOOL_SYSTEMMEM, &pSysMem, nullptr);
    ...
    out.pixels.assign(static_cast<const uint8_t*>(locked.pBits), static_cast<const uint8_t*>(locked.pBits) + static_cast<size_t>(locked.Pitch) * desc.Height);
...
bool CMultiModalCapture::CaptureMultiModalFrame(const std::string& rgbPath, const std::string& segPath, const std::string& depthPath, bool saveRgbToVideo,
                                                bool saveSegToVideo, bool saveDepthToVideo, int jpegQuality)
{
    auto t0 = std::chrono::steady_clock::now();
    SegDiagLog(m_bDiagLogsEnabled, "[Seg/Capture] enter frame=%llu rgb='%s' seg='%s' depth='%s' vRGB=%d vSeg=%d vDepth=%d q=%d\n",
               static_cast<unsigned long long>(m_FrameIndex), rgbPath.c_str(), segPath.c_str(), depthPath.c_str(), saveRgbToVideo ? 1 : 0,
               saveSegToVideo ? 1 : 0, saveDepthToVideo ? 1 : 0, jpegQuality);
    ...
    if (wantSeg && !RenderSegmentation(m_pDevice))
    {
        OutputDebugString("[CMultiModalCapture] Seg snapshot failed\n");
        return false;
    }
    if (wantDepth && !RenderDepthMap(m_pDevice))
    {
        OutputDebugString("[CMultiModalCapture] Depth snapshot failed\n");
        return false;
    }
    ...
    if (!segPath.empty())
    {
        ...
        SegDiagLog(m_bDiagLogsEnabled, "[Seg/Capture] readback Seg ok=%d pitch=%d %ux%u  px@mid-row: %06X %06X %06X\n", ok ? 1 : 0, segRead.pitch,
                   segRead.width, segRead.height, px0, pxMid, pxLast);
    }
    if (!depthPath.empty())
    {
    ...
    auto isJpegPath = [](const std::string& p) -> bool
    {
        ...
        return lower.size() >= 4 &&
               (lower.compare(lower.size() - 4, 4, ".jpg") == 0 || (lower.size() >= 5 && lower.compare(lower.size() - 5, 5, ".jpeg") == 0));
    };
    ...
    {
        std::lock_guard<std::mutex> lock(m_PendingMutex);
        nPending = m_PendingSaves.size();
    }
    SegDiagLog(m_bDiagLogsEnabled, "[Seg/Capture] DONE(queued) frame=%llu elapsedMs=%lld pending=%zu\n", static_cast<unsigned long long>(m_FrameIndex),
...
void CMultiModalCapture::OnPresent(IDirect3DDevice9* pDevice)
{
    ...
    SegDiagLog(m_bDiagLogsEnabled,
               ...
               static_cast<unsigned long long>(m_FrameIndex), m_bSegmentationEnabled ? 1 : 0, static_cast<void*>(pDevice), static_cast<void*>(m_pDevice),
               (pDevice == m_pDevice) ? 1 : 0, m_SegStats.emitCalls, m_SegStats.drawsFired, m_bSegSurfaceNeedsClear ? 1 : 0);
    ...
    if (m_pRGBSurface)
    {
        ...
        if (SUCCEEDED(hrBB) && pBackBuffer)
        {
            ...
            SegDiagLog(m_bDiagLogsEnabled, "[Seg/Present] backbuffer->RGB GetBB=0x%08X BB=%p StretchRect=0x%08X\n", static_cast<unsigned>(hrBB),
                       static_cast<void*>(pBackBuffer), static_cast<unsigned>(hrSR));
            pBackBuffer->Release();
        }
        else
        {
    ...
    {
        HRESULT hrSegSR = pDevice->StretchRect(m_pSegmentationSurface, nullptr, m_pSegmentationSnapshot, nullptr, D3DTEXF_NONE);
    ...
    if (m_bDiagLogsEnabled && m_bSegmentationEnabled && (m_SegStats.emitCalls > 0 || m_SegStats.drawsFired > 0))
    {
        ...
        char buf[256];
        snprintf(buf, sizeof(buf), "[SegDiag] emit=%d enabled=%d scene=%d res=%d size=%d fired=%d uniqueCols=%zu\n", m_SegStats.emitCalls,
                 m_SegStats.passedEnabled, m_SegStats.passedScene, m_SegStats.passedResources, m_SegStats.passedSizeGate, m_SegStats.drawsFired,
        ...
        if (!m_SegRTSizeBucket.empty())
        {
            ...
            std::sort(sorted.begin(), sorted.end(), [](const std::pair<uint64_t, int>& a, const std::pair<uint64_t, int>& b) { return a.second > b.second; });
            std::string line = "[SegDiag] RTsizes:";
            for (size_t i = 0; i < sorted.size() && i < 5; ++i)
            {
                char b2[64];
                snprintf(b2, sizeof(b2), " %ux%u=%d", static_cast<unsigned>(sorted[i].first >> 32), static_cast<unsigned>(sorted[i].first & 0xFFFFFFFFu),
                         sorted[i].second);
                line += b2;
            }
            line += "\n";
...
// draw in that case.
static IDirect3DStateBlock9* SetupSegmentationState(IDirect3DDevice9* pDevice, IDirect3DSurface9* pSegSurface, IDirect3DSurface9* pSegDS,
                                                    IDirect3DPixelShader9* pConstPS, CTextureRegistry& registry, std::unordered_set<uint32_t>& uniqueColorsOut,
                                                    bool traceThisDraw)
{
    ...
    const float psConstant[4] = {((color >> 16) & 0xFF) / 255.0f, ((color >> 8) & 0xFF) / 255.0f, (color & 0xFF) / 255.0f, 1.0f};
    ...
    SegDiagLog(traceThisDraw, "[Seg/Setup] SetRT=0x%08X SetDS=0x%08X SetPS=0x%08X SetConst=0x%08X\n", static_cast<unsigned>(hrRT), static_cast<unsigned>(hrDS),
...
static void EmitSegmentationCommon(IDirect3DDevice9* pDevice, IDirect3DSurface9* pSegSurface, IDirect3DSurface9* pSegDS, IDirect3DPixelShader9* pConstPS,
                                   CTextureRegistry& registry, bool& needsClearFlag, std::unordered_set<uint32_t>& uniqueColorsOut, bool traceThisDraw,
                                   DrawFn doDraw)
{
    ...
    IDirect3DStateBlock9* pBlock = SetupSegmentationState(pDevice, pSegSurface, pSegDS, pConstPS, registry, uniqueColorsOut, traceThisDraw);
    if (!pBlock)
    {
    ...
    if (needsClearFlag)
    {
        ...
        HRESULT hrClear = pDevice->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(255, 0, 255), 1.0f, 0);
    ...
    if (traceThisDraw)
    {
        ...
        SegDiagLog(traceThisDraw, "[Seg/Emit] AFTER-Apply rt=%p ds=%p  (expected rt=%p ds=%p)  leakRT=%d leakDS=%d\n", static_cast<void*>(pAfterRT),
                   static_cast<void*>(pAfterDS), static_cast<void*>(pSavedRT), static_cast<void*>(pSavedDS), (pAfterRT != pSavedRT) ? 1 : 0,
                   (pAfterDS != pSavedDS) ? 1 : 0);
        if (pAfterRT)
            pAfterRT->Release();
        if (pAfterDS)
            pAfterDS->Release();
...
{
    return g_bInGTAScene.load(std::memory_order_acquire) && !g_bInMTAScene.load(std::memory_order_acquire);
}
...
static bool IsDrawingToFullSizeRT(IDirect3DDevice9* pDevice, UINT expectedW, UINT expectedH)
{
    ...
    if (FAILED(pDevice->GetRenderTarget(0, &pCurRT)) || !pCurRT)
        return false;
    D3DSURFACE_DESC desc = {};
...
static void TraceDrawDetails(const char* tag, IDirect3DDevice9* pDevice, unsigned int primType, unsigned int primCount)
{
    ...
    snprintf(buf, sizeof(buf),
             ...
             tag, texName, primType, primCount, rtW, rtH, vp.X, vp.Y, vp.Width, vp.Height, vp.MinZ, vp.MaxZ, static_cast<unsigned long>(zEnable),
             static_cast<unsigned long>(zWrite), static_cast<void*>(pTex));
    OutputDebugStringA(buf);
    static std::ofstream s_drawLog("seg_diag.log", std::ios::out | std::ios::app | std::ios::binary);
    if (s_drawLog.is_open())
    {
        s_drawLog << buf;
        s_drawLog.flush();
    }
...
void CMultiModalCapture::EmitSegmentationDraw(IDirect3DDevice9* pDevice, unsigned int primitiveType, unsigned int startVertex, unsigned int primitiveCount)
{
    m_SegStats.emitCalls++;
    if (!m_bSegmentationEnabled)
        return;
    ...
    if (!pDevice || !m_pSegmentationSurface || !m_pSegDepthStencil || !m_pConstantColorShader)
        return;
    m_SegStats.passedResources++;

    RecordCurrentRTSize(pDevice, m_SegRTSizeBucket);

    if (!IsDrawingToFullSizeRT(pDevice, static_cast<UINT>(m_iCaptureWidth), static_cast<UINT>(m_iCaptureHeight)))
        return;
    ...
    EmitSegmentationCommon(pDevice, m_pSegmentationSurface, m_pSegDepthStencil, m_pConstantColorShader, m_TextureRegistry, m_bSegSurfaceNeedsClear,
                           m_SegUniqueColorsThisFrame, traceThis,
                           [&](IDirect3DDevice9* d)
                           {
                               HRESULT hr = d->DrawPrimitive(static_cast<D3DPRIMITIVETYPE>(primitiveType), startVertex, primitiveCount);
                               SegDiagLog(traceThis, "[Seg/Emit] DrawPrimitive hr=0x%08X type=%u startV=%u cnt=%u\n", static_cast<unsigned>(hr), primitiveType,
                                          startVertex, primitiveCount);
                           });
    m_SegStats.drawsFired++;
}

void CMultiModalCapture::EmitSegmentationDrawIndexed(IDirect3DDevice9* pDevice, unsigned int primitiveType, int baseVertexIndex, unsigned int minVertexIndex,
                                                     unsigned int numVertices, unsigned int startIndex, unsigned int primitiveCount)
{
    ...
    EmitSegmentationCommon(pDevice, m_pSegmentationSurface, m_pSegDepthStencil, m_pConstantColorShader, m_TextureRegistry, m_bSegSurfaceNeedsClear,
                           ...
                           [&](IDirect3DDevice9* d)
                           {
                               HRESULT hr = d->DrawIndexedPrimitive(static_cast<D3DPRIMITIVETYPE>(primitiveType), baseVertexIndex, minVertexIndex, numVertices,
